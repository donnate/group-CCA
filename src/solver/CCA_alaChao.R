...
lasso_formulation <- function(Sigma_x, Sigma_y, Sigma_xy, H,
                              G, lambda, eta = 0.5,
...
group_lasso <- function(Sigma_x, Sigma_xy, V,
...
sparseCCA_Chao<- function(X, Y, k, lambda1, lambda2, lambda3,  
                       eta=1, 
  ...
  while(converged == FALSE){
    lasso_sol <- lasso_formulation(Sigma_x, Sigma_y, Sigma_xy, old.H,
  ...
  res = group_lasso(Sigma_x, Sigma_xy, svd_A$v,
                    lambda2,
                    max_k = max_k, max.iter=max.iter,
                    verbose = verbose)
  res_y = group_lasso(Sigma_y, t(Sigma_xy), svd_A$u,
                    lambda3,
                    max_k = max_k, max.iter=max.iter,
                    verbose = verbose)
...
sparseCCA.CV<-function(X, Y, D, rank, n.cv=5,
  ...
  Fit.sparseCCA <- sparseCCA(X=Xtrain, Y=Ytrain, rank, cv.results$lambda1[indx], 
...
sparseCCA.cv.lambda2_2<-function(V,Xtrain,Ytrain, 
  ...
  Fit.genCCA <- sparseCCA(Xtrain, Ytrain, rank, V, lambdaxfixed, 
                          eta=eta,max_k=max_k, 
                          zero_threshold = zero_threshold, 
                          epsilon=epsilon,
                          max.iter = max.iter,
