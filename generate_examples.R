...
generate_example_sparse_product <- function(n, p1, p2,
  ...
  return(list(Sigma=Sigma, Sigma0=Sigma0,
              S = S, sigma0hat =  sigma0hat, Mask= Mask,
              X=X, Y = Y, Data=Data, 
              u=sqrtm(Sigma[1:p1, 1:p1])$Binv %*% GT$u, 
              v=sqrtm(Sigma[(p1+1):(p1+p2), (p1+1):(p1+p2)])$Binv %*% GT$v, 
              Xnew = Xnew, Ynew=Ynew,
              Sigmax=Sigmax, Sigmay=Sigmay
  ))
}
...
generate_example_sparse_X <- function(n, p1, p2,
                                      r_pca = 3,
                                      nnzeros = 5,
                                      theta = diag(c(0.9,  0.8)),
                                      lambda_pca = 1,
  ...
  if (r_pca > 0){
    ...
    ### Same for Sigma_y
    u2 <- matrix(0, pp[2], r_pca)
    u2[s_pca2, 1:r_pca] <- matrix(runif( q * r_pca,max = 3, min=1), nrow=q)  * matrix(sample(c(-1,1), q*r_pca, replace=TRUE), nrow=1:q, ncol=r_pca)
    u2[s_pca2, 1:r_pca] <- u2[s_pca2, 1:r_pca] %*% (sqrtm(t(u2[s_pca2, 1:r_pca]) %*% u2[s_pca2, 1:r_pca])$Binv)
    T2 <- u2 %*% diag(Lambda_pca) %*% t(u2)
    if (normalize_diagonal){
      diag(T2) <- 1
  ...
  #precision_y =  Sigma[(p1 + 1):(p1 + p2), (p1 + 1):(p1 + p2)]
  Tss_v <- Sigma[(p1 + 1):(p1 + p2), (p1 + 1):(p1 + p2)]
  prod_v <- matrix(0, pp[2], r)
  prod_v[s, 1:r] <- as.matrix(runif( nnzeros * r,max = 3, min=1), nrow=nnzeros)  * as.matrix(sample(c(-1,1), nnzeros*r, replace=TRUE), nrow=nnzeros)
  v <- prod_v %*% (sqrtm(t(prod_v[s, 1:r]) %*% Tss_v %*% prod_v[s, 1:r])$Binv)
  #v = precision_y %*% prod_v
