...
transform = function(Y, Ky){
  ...
  tilde = ED$vectors[,1:r] %*% diag(1/sqrt(ED$values[1:r])) %*% t(ED$vectors[,1:r])
  notilde = ED$vectors[,1:r] %*% diag(sqrt(ED$values[1:r])) %*% t(ED$vectors[,1:r])
  return(list(tilde = tilde, notilde = notilde, mu = mu))
}
