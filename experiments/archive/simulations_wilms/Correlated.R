...
source('../alternative_methodss/Witten_CrossValidation.R')
source('../alternative_methods/Waaijenborg.R')


##############################################################
# Simulation study Canonical Correlations: Correlated design #
##############################################################
...
for (i in 1:q){
  for (j in 1:q){
    SIGMA_YY[i,j]<-0.7^(abs(i-j))
  }
}
SIGMA_YY[4:10,]<-SIGMA_YY[,4:10]<-0
diag(SIGMA_YY)<-1
rhos<-c(1/2,1/2,0,0,0)
dimension<-length(which(rhos!=0))
SIGMA_XY<-matrix(0,nrow=p,ncol=q)
SIGMA_XY[1,1]<-1/2
SIGMA_XY[2,2]<-1/2
SIGMA_BIG<-rbind(cbind(SIGMA_XX,SIGMA_XY),cbind(t(SIGMA_XY),SIGMA_YY))

# Calculate true canonical vectors
...
for (isim in 1:Nsim){
  ...
  ### SAR - BIC
  FIT_SAR_BIC<-SparseCCA(X=X,Y=Y,rank=rank,lambdaAseq=seq(from=0.2,to=0,length=10),lambdaBseq=seq(from=0.2,to=0,length=10),max.iter=100,conv=10^-2,selection.criterion=1,n.cv=5)
  ...
  ### SAR - CV
  FIT_SAR_CV<-SparseCCA(X=X,Y=Y,rank=rank,lambdaAseq=seq(from=0.2,to=0,length=10),lambdaBseq=seq(from=0.2,to=0,length=10),max.iter=100,conv=10^-2,selection.criterion=2,n.cv=5)
  ...
  Waaijenborg_Delta<-Waaijenborg(X=X, Y=Y,
