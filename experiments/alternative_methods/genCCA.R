...
genCCA<-function(X,Y,lambdaAseq=seq(from=1,to=0.01,by=-0.01),lambdaBseq=seq(from=1,to=0.01,by=-0.01),rank,selection.criterion=1,n.cv=5,A.initial=NULL,B.initial=NULL,max.iter=20,conv=10^-2){
  ...
  for (i.r in 1:rank){
    ...
    while( (it<max.iter) & (diff.obj>conv) ){
      ...
      FIT.A<-alternating.regression(Xreg=X_data,Yreg=Y_data%*%B.STARTING,lambdaseq=lambdaAseq,selection.criterion=selection.criterion,n.cv=n.cv)
      AHAT_FINAL<-FIT.A$COEF_FINAL
      lambdaA_ALL[it,i.r]<-FIT.A$LAMBDA_FINAL
      
      # Estimating B conditional on A
      FIT.B<-alternating.regression(Xreg=Y_data,Yreg=X_data%*%AHAT_FINAL,lambdaseq=lambdaBseq,selection.criterion=selection.criterion,n.cv=n.cv)
    ...
    if (i.r==1){# FIRST DIMENSION
      ...
      FIT.Aorig<-alternating.regression(Yreg=Uhat,Xreg=X,lambdaseq=lambdaAseq,selection.criterion=selection.criterion,n.cv=n.cv)
      ALPHAhat<-FIT.Aorig$COEF_FINAL
      lambdaA_FINAL<-FIT.Aorig$LAMBDA_FINAL
      
      # B expressed in terms of original data set Y
      FIT.Borig<-alternating.regression(Yreg=Vhat,Xreg=Y,lambdaseq=lambdaBseq,selection.criterion=selection.criterion,n.cv=n.cv)
