...
for (n in c(N)){
  for (psize in c(0.25 *n, 0.5*n, 0.75*n, n, 1.5 *n , 2 *n, 5 *n)){
    for (power in c(0.1, 0.5, 1, 1.5, 2)){
      for(example_type in c("localized", "gamma_sparse")){
        p1=as.integer(psize); p2=as.integer(psize)
        #nnz = ceil(sparsity * n)
        print(c(n, p1, p2))
        p = p1+ p2
        ZERO_THRESHOLD = 0.001 * sqrt(1/p1)
        max_attempts <- 3
        attempt <- 1
        ...
        while (attempt <= max_attempts) {
          ...
          tryCatch({
            ...
            if (example_type =="localized"){
              print(example_type)
              example <- generate_localized_example(n=n, p1=p1,  
                                                    nnzeros = 2,
                                                    theta = diag( c(0.9,  0.8)),
                                                    r=2, type_graph=type_graph, 
                                                    power=power, probs=PROBA,
                                                    threshold_limit=0.8, order=ORDER)
            }else{
              print(example_type)
              example <- generate_gamma_sparse_example(n=n, p1=p1,  
                                                       nnzeros = 2,
                                                       theta = diag( c(0.9,  0.8)),
                                                       r=2, type_graph=type_graph, 
                                                       power=power, probs=PROBA,
                                                       threshold_limit=0.8, order=1)
            }
        ...
        simp_a  = example$a
        simp_a[which(abs(simp_a)<1e-4)] = 0
        final_nnz = sum(apply(simp_a^2,1,sum)>0)
        
        print("here")
        #param1 = 10^(seq(-5, 3, by = 0.2))
        max1 = 500 * sqrt(log(p1)/n)
        min1 = 0.02 * sqrt(log(p1)/n)
        param1 = 10^(seq(-1, 2, by = 0.2))
        #param1= c(0.1) 
        ...
        for (adaptive in c(TRUE, FALSE)){
          for (create_folds in c(TRUE, FALSE)){
            ...
            #Uhat = Uhat + example$Data %*% matrix(1, p, 1) %*% t(matrix(1, p, 1) )
            #Uhat[which(abs(Uhat)<ZERO_THRESHOLD)]=0
            #norm(Uhat - example$a)
            temp <- data.frame("method" = name_method,
                               ...
                               "nb_discoveries" = sum(apply(Uhat^2, 1, sum)>ZERO_THRESHOLD),
                               "param1" = res$lambdax,
                               "param2" = res$lambday,
